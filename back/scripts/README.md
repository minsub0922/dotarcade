...
# Record one continuous meeting -> game -> 20-agent feedback run.
DOTCADE_URL=http://127.0.0.1:5173/ \
...
The continuous recorder only succeeds after all 20 reports, the summary, and the persisted game-pack feedback are present. Use `DOTCADE_RECORD_NAME` while recording and the same `DOTCADE_FULL_RUN_NAME` while assembling to keep multiple takes.
