...
export function getStudioMilestones({ games = [], meeting = null, arcade = null, studio = null, map = 'office', taskActivity = null } = {}) {
  ...
  const team = withState({
    ...
    guide: '팀원 근처에서 E로 대화하거나, 소품을 E로 집은 뒤 F로 던져 맞혀 보세요.',
  ...
  const play = withState({
    ...
    guide: '게임팩에서 원하는 게임을 고른 뒤 ‘플레이’를 누르세요.',
  ...
  const create = withState({
    ...
    guide: initialMeetingActive ? '‘회의 열기’를 눌러 진행 중인 제작 단계로 돌아가세요.' : '회의실에 도착하면 안건을 입력하고 ‘회의 시작’을 누르세요.',
  ...
  const interrupt = withState({
    ...
    guide: '제작 회의가 진행 중일 때 ‘회의 일시정지’를 눌러 현재 컨텍스트를 저장하세요.',
  ...
  const evaluation = withState({
    ...
    guide: evaluationAction === MILESTONE_ACTION.START_PLAYTEST
      ? '오락실에 배포하고 AI 손님 20명의 평가를 시작하세요.'
      : evaluationAction === MILESTONE_ACTION.VIEW_REPORT ? '완료된 평가 리포트를 열어 결과와 개선점을 확인하세요.' : '진행 중인 AI 플레이테스트 현장을 확인하세요.',
  ...
  const improvement = withState({
    ...
    guide: upgradeMeetingActive ? '‘개선 회의 열기’를 눌러 진행 중인 업데이트로 돌아가세요.' : '평가 피드백을 확인하고 개선 회의를 시작하세요.',
