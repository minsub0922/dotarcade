...
export default function MeetingStart({ onStart }) {
  ...
  function go() {
    ...
    onStart(agenda.trim(), up || null)
  }

  return (
    <div className="modal-back" onClick={e => e.target === e.currentTarget && closePanel()}>
      <div className="modal">
        <div className="modal-head"><b>📋 회의 안건 제출</b><button className="x" onClick={closePanel}>✕</button></div>
        <div className="detail-body">
          <div className="tiny muted">팀장으로서 안건을 제시하면 팀원 5명이 회의실에 모여 BMAD 파이프라인(리서치→토론→PRD→디자인→설계→구현→QA→릴리즈)으로 <b>실제 동작하는 도트 게임</b>을 만들어냅니다.</div>
          <div className="field">
            <label>회의 종류</label>
          ...
          <div className="actions">
            <button className="primary" onClick={go} disabled={!agenda.trim()}>🚀 회의 소집</button>
          </div>
        </div>
      </div>
    </div>
